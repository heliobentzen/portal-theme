...
function portal_breadcrumb() {
    ...
    if (!is_front_page() || is_paged()) {
        ...
        echo '<li class="breadcrumb-item"><a href="' . esc_url( $homeLink ) . '">' . esc_html( $nomesite ) . '</a> '.$sep. '</li> ';

        if (is_home()) {
            echo $before . esc_html( get_the_title(get_option( 'page_for_posts' )) ) . $after;
        } elseif (is_category()) {
            global $wp_query;
            ...
            if ($thisCat->parent != 0) {
                echo wp_kses_post( get_category_parents($parentCat, true, $sep) );
            }
            echo $before . esc_html( single_cat_title('', false) ) . $after;
        } elseif (is_search()) {
            echo $before . 'Resultado da pesquisa por: "' . esc_html( get_search_query() ) . '"' . $after;
        } elseif (is_day()) {
            ...
            if (get_post_type() != 'post') {
                ...
                echo '<li class="breadcrumb-item"><a href="' . esc_url( $homeLink . '/' . $slug['slug'] . '/' ) . '">' . esc_html( $post_type->labels->name ) . '</a></li> ';
                echo $before . esc_html( get_the_title() ) . $after;
            } else {
                $cat = get_the_category();
                $cat = $cat[0];
                echo '<li class="breadcrumb-item">'.wp_kses_post( get_category_parents($cat, true, $sep) ).'</li>';
                echo $before . esc_html( get_the_title() ) . $after;
            }
        } elseif (!is_single() && !is_page() && get_post_type() != 'post' && !is_404()) {
            if (is_tax('curso_modalidade') && !isset($_POST['curso_modalidade'])) {
                echo '<li class="breadcrumb-item"><a href="' . esc_url( get_post_type_archive_link( 'curso' ) ) . '">' . __('Cursos', 'ifrs-portal-theme') . '</a></li>';
                echo $before . 'Modalidades' . $after;
                echo $before . esc_html( single_term_title('', false) ) . $after;
                ...
                echo '<li class="breadcrumb-item"><a href="' . esc_url( get_post_type_archive_link( 'concurso' ) ) . '">' . __('Concursos', 'ifrs-portal-theme') . '</a></li>';
                echo $before . esc_html( single_term_title('', false) ) . $after;
            } elseif (is_tax('documento_type') && !isset($_POST['documento_type'])) {
                echo '<li class="breadcrumb-item"><a href="' . esc_url( get_post_type_archive_link( 'documento' ) ) . '">' . __('Documentos', 'ifrs-portal-theme') . '</a></li>';
                echo $before . esc_html( single_term_title('', false) ) . $after;
            } elseif (is_tax('documento_origin') && !isset($_POST['documento_origin'])) {
                echo '<li class="breadcrumb-item"><a href="' . esc_url( get_post_type_archive_link( 'documento' ) ) . '">' . __('Documentos', 'ifrs-portal-theme') . '</a></li>';
                echo $before . esc_html( single_term_title('', false) ) . $after;
            } elseif (is_tax('edital_category') && !isset($_POST['edital_category'])) {
                echo '<li class="breadcrumb-item"><a href="' . esc_url( get_post_type_archive_link( 'edital' ) ) . '">' . __('Editais', 'ifrs-portal-theme') . '</a></li>';
                echo $before . esc_html( single_term_title('', false) ) . $after;
            } elseif (is_tax('edital_status') && !isset($_POST['edital_status'])) {
                echo '<li class="breadcrumb-item"><a href="' . esc_url( get_post_type_archive_link( 'edital' ) ) . '">' . __('Editais', 'ifrs-portal-theme') . '</a></li>';
                echo $before . esc_html( single_term_title('', false) ) . $after;
            } else {
                echo $before . esc_html( post_type_archive_title('', false) ) . $after;
            }
        } elseif (is_tax('escopo')) {
            echo $before . esc_html( single_term_title('Not&iacute;cias para ', false) ) . $after;
        } elseif (is_attachment()) {
            $parent = get_post($post->post_parent);
            $cat    = get_the_category($parent->ID);
            $cat    = $cat[0];
            echo wp_kses_post( get_category_parents($cat, true, $sep) );
            ...
            while ($parent_id) {
                ...
                $breadcrumbs[] = '<li class="breadcrumb-item"><a href="' . esc_url( get_permalink($page->ID) ) . '">' . esc_html( get_the_title( $page->ID ) ) . '</a>' . $sep . '</li>';
            ...
            echo $before . 'Posts com a tag "' . esc_html( single_tag_title('', false) ) . '"' . $after;
        } elseif (is_author()) {
            global $author;
